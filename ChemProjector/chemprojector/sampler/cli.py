...
@click.command()
@click.option("--input", "-i", type=_input_mols_option, 
              #required=True
              default="data/example.csv")
@click.option("--output", "-o", type=click.Path(exists=False, path_type=pathlib.Path), 
              #required=True
              default="results/example.csv")
@click.option(
    "--model-path",
    "-m",
    type=click.Path(exists=True, path_type=pathlib.Path),
    default="data/trained_weights/original_default.ckpt",
    #required=True,
)
@click.option(
    "--rxn-matrix-path",
...
@click.option("--time-limit", type=int, default=18000)
def main(
    input: list[Molecule],
    output: pathlib.Path,
