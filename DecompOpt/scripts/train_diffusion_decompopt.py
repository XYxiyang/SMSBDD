...
from models.decompopt import DecompScorePosNet3D
...
if __name__ == '__main__':
    ...
    # Transforms
    config.model.add_retrieved_feat = getattr(config.data.transform, 'add_retrieved_feat', False) #True in original config
    protein_featurizer = trans.FeaturizeProteinAtom() #
    ligand_featurizer = trans.FeaturizeLigandAtom(config.data.transform.ligand_atom_mode,
                                                  config.model.prior_types) #basic and ref_prior
    ...
    decomp_indicator = trans.AddDecompIndicator(
        ...
        add_retrieved_feat=getattr(config.data.transform, 'add_retrieved_feat', False)
    )
    transform_list = [
    ...
    if getattr(config.model, 'bond_diffusion', False):
        transform_list.append(
            trans.FeaturizeLigandBond(mode=config.data.transform.ligand_bond_mode, 
                                        set_bond_type=True)
        )
        if getattr(config.data.transform, 'add_retrieved_feat', False):
            transform_list.append(
                trans.FeaturizeArmBond(
                    bond_featurizer=trans.FeaturizeLigandBond(
                        mode='fc', set_bond_type=True))
            )
    if getattr(config.data.transform, 'add_retrieved_feat', False):
        arm_featurizer = trans.FeaturizeArmAtom(protein_featurizer, ligand_featurizer, padding_protein_feature_dim = decomp_indicator.protein_feature_dim)
        transform_list.append(arm_featurizer)
    ...
    collate_exclude_keys = ['ligand_nbh_list', 'pocket_atom_masks', 'pocket_prior_masks', \
                            'scaffold_prior', 'arms_prior', 'retrieved_frags',]
    ...
    def train(it):
        ...
        try:
            for _ in range(config.train.n_acc_batch):
                ...
                assert isinstance(model, DecompScorePosNet3D)
